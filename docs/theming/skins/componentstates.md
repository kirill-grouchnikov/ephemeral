...
Ephemeral has a number of predefined component states to cover most action-based controls such as buttons, check boxes and menu items. In addition, custom component states can be defined to create finer grained mapping between arbitrary states of controls and specific color tokens bundles in custom skins.
...
When the matching algorithm cannot find a sufficiently close match, the skinning layer will fall back on one of the base color tokens - active, muted or neutral. States with `ComponentStateFacet.Enable` in their `off` list will also use the matching `alpha` for surface, outline or content colors. The `ComponentState.Enabled` will fall back to either muted or neutral color tokens. The rest of the states will fall back to the active color tokens. This fallback behavior can be overriden as necessary by specifying fallback color tokens in your skin definition.
