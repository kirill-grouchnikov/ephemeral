...
Note how the selected button uses light text on dark background, while the rollover selected button uses dark text on light background. If your UI uses the Magellan skin (or any other skin that uses a mix of light and dark color tokens), an Ephemeral skin will do the right thing to animate all relevant parts of your UI as the user interacts with it.
 ...
 * `IconFilterStrategy.ThemedFollowColorTokens` - filter the icon to use the Ephemeral color tokens from the currently set skin that matches the component in its current state

`ThemedFollowColorTokens` which works best for multi-color / multi-tone icons and Ephemeral skins with multi-color color tokens. Here is the same UI with this icon filter strategy applied on enabled and disabled component states:
...
* Nebula Amethyst skin is used to query the color tokens to theme the original icon that remains crisp and legible.
